				...
				<div id="ctablediv">
					...
					<c:forEach items="${commlist}" var="comm">
						<table class="ctable${comm.c_no}" style="word-break: break-all" id = "commtablelist">
							...
							<tr>
								<td class="commpro_radius"><img src="./resources/img/프로필.png"><b>${comm.id}</b></td>
								<td width="5%"><button type="button" class="delete${comm.c_no} postcommbut" onclick="delcomm(${comm.c_no})">삭제</button>
								<button type="button" class="report${comm.c_no} postcommbut" onclick="alert('report')">신고</button></td>
							</tr>
							<tr>
								<td class="comm_date">${comm.c_date}</td>
							</tr>
...
<script>
	...
	$(document).ready(function(){
	  //실행내용
		...
		/* $("#ctablediv").load(window.location.href + " #commtablelist"); */
		})


	
	function tablerefresh() {
		$('#ctablediv').load(window.location.href+' #ctablediv');
		console.log("이거되는거임?")
	}
				...
				cache : false,
				 success: function(data) {
		            ...
		            $.each(commlist, function(index, comm) {
		                ...
		                html += '<tr>';
		                html += '<td class="commpro_radius"><img src="./resources/img/프로필.png"><b>' + comm.id + '</b></td>';
		                ...
		                html += '</tr>';
		                html += '<tr>';
		                html += '<td class="comm_date">' + comm.c_date + '</td>';
		                html += '</tr>';
		                html += '<tr>';
